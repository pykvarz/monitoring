...
class HostManager:
    """Менеджер операций с хостами (via DataManager)"""
    
    @staticmethod
    def add_host(parent, groups: List[str], data_manager: DataManager) -> bool:
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            # TODO: Проверка дубликатов через DataManager?
            # Пока доверяем валидации или добавим позже
            
            if data_manager.add_host(new_host):
                return True
            else:
    ...
    @staticmethod
    def delete_selected(parent, table_model, data_manager: DataManager) -> None:
        ...
        if reply == QMessageBox.Yes:
            ...
            for hid in ids_to_delete:
                data_manager.delete_host(hid)
    
    @staticmethod
    def edit_host(parent, row: int, table_model, groups: List[str], data_manager: DataManager) -> None:
        """Редактирование узла"""
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if not data_manager.update_host_info(edited_host):
                QMessageBox.warning(parent, "Ошибка", "Не удалось сохранить изменения")

    @staticmethod
    def toggle_maintenance_selected(parent, table_model, data_manager: DataManager) -> None:
        ...
        hosts = data_manager.get_hosts_by_ids(ids)
        for host in hosts:
            ...
            data_manager.update_host_status(host.id, new_status, None)

    @staticmethod
    def toggle_maintenance(parent, row: int, table_model, data_manager: DataManager) -> None:
    ...
    @staticmethod
    def change_group_selected(parent, table_model, groups: List[str], data_manager: DataManager) -> None:
        ...
        for host in hosts:
            ...
            data_manager.update_host_info(host)

    @staticmethod
    def toggle_notifications_selected(parent, table_model, data_manager: DataManager) -> None:
    ...
    @staticmethod
    def toggle_notifications(parent, row: int, table_model, data_manager: DataManager) -> None:
        ...
        hosts = data_manager.get_hosts_by_ids([host.id])
        if hosts:
            h = hosts[0]
            h.notifications_enabled = not h.notifications_enabled
            data_manager.update_host_info(h)
